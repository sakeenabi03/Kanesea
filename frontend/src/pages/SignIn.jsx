...
function SignIn() {
    ...
    const handleSignInGoogleAuth = async () => {
        const provider = new GoogleAuthProvider()
        const result = await signInWithPopup(auth, provider)
        try {
            const {data} = await axios.post(`${serverUrl}/api/auth/signin-google-auth`, {
    ...
    return (
        <div className={`min-h-screen w-full transition-opacity duration-1000 flex flex-col items-center bg-white ${loaded ? "opacity-100" : "opacity-0"}`}>
            ...
            {/* Content */}
            <div className="lg:px-20 px-5 py-8 rounded-3xl lg:w-[600px] md:w-[500px] w-[350px] flex flex-col justify-start bg-gray-100 shadow-2xl mb-20">
                ...
                <div className="relative w-full flex flex-col">
                    ...
                    <input type="email" name="email" className="w-full px-4 py-3 text-black bg-gray-100 rounded-xl border border-gray-400 focus:outline-none focus:ring-2 focus:ring-black mt-2" placeholder="Enter Your Email" onChange={(e)=>setEmail(e.target.value)} value={email} required />
                </div>
                ...
                <div className="flex justify-end text-sm text-black mt-2">
                    <div className="hover:underline cursor-pointer" onClick={()=>navigate("/forgot-password")}>
                        Forgot Password?
                    </div>
                </div>
                ...
                <div className="w-full max-w-sm mx-auto text-center mt-6">
                    ...
                    <button type="button" className="w-full mt-4 flex items-center justify-center gap-2 py-3 text-black rounded-full hover:bg-white hover:text-black transition font-medium cursor-pointer border border-black" onClick={handleSignInGoogleAuth}>
                        <FcGoogle size={22}/>
                    ...
                    <button type="button" className="w-full py-3 bg-gray-200 text-gray-800 rounded-full hover:bg-gray-300 transition font-medium cursor-pointer" onClick={()=>navigate("/signup")}>
                        Sign up
                    </button>
                </div>
            </div>
