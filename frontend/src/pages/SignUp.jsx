...
function SignUp() {
    ...
    const handleGoogleAuth = async () => {
        ...
        try {
            const {data} = await axios.post(`${serverUrl}/api/auth/signup-google-auth`, {
        ...
        <div className={`min-h-screen w-full transition-opacity duration-1000 flex flex-col items-center justify-center bg-white ${loaded ? "opacity-100" : "opacity-0"}`}>
            ...
            <div className="lg:px-20 px-5 py-8 rounded-3xl lg:w-[600px] md:w-[600px] w-[350px] flex flex-col justify-start bg-gray-100 shadow-2xl mb-20">
                ...
                <div className="w-full max-w-sm mx-auto text-center mt-6">
                    ...
                    <button type="button" className="w-full mt-4 flex items-center justify-center gap-2 py-3 text-black rounded-full hover:bg-white hover:text-black transition font-medium cursor-pointer border border-black" onClick={handleGoogleAuth}>
                        <FcGoogle size={22}/>
                    ...
                    <button type="button" className="w-full py-3 bg-gray-200 text-gray-800 rounded-full hover:bg-gray-300 transition font-medium cursor-pointer" onClick={()=>navigate("/signin")}>
                        Log in
                    </button>
                </div>
            </div>
